...
describe('Regenerate Service', () => {
  ...
  describe('RST-003: 内存释放效率>80%', () => {
    it('应释放超过80%的Transient内存', async () => {
      ...
      expect(result.preservedKeys.length).toBeGreaterThanOrEqual(2);
    });
  ...
  describe('保留键功能', () => {
    ...
    it('应保留State Machine相关键', async () => {
      ...
      expect(result.preservedKeys).toContain('state:current');
      expect(result.preservedKeys).toContain('state:history');
    });
  });
