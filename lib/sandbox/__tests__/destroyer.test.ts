...
import { Destroyer, DestructionStatus, DestroyerConfig } from '../destroyer';
...
describe('Destroyer - 自毁模块', () => {
  ...
  describe('DEST-001: 自毁验证', () => {
    ...
    it('应正确计算销毁耗时', async () => {
      ...
      await destroyer.schedule(executionId, 50);
      await new Promise((resolve) => setTimeout(resolve, 100));

      const task = destroyer.getTask(executionId);
      expect(task?.result).toBeDefined();
      expect(task?.result?.durationMs).toBeGreaterThanOrEqual(0);
    });
  });
