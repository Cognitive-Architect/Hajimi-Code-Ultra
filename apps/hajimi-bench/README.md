...
├──────────────┼───────────────────┼──────────────────────────┤
│ • hajimi-diff│ • ai-chat/        │ • 压缩率评分              │
│ • gzip       │ • code/           │ • 速度评分                │
│ • raw        │ • git/            │ • 内存监控                │
│              │ • extreme/        │ • 正确性校验 (SHA256)      │
└──────────────┴───────────────────┴──────────────────────────┘
```
...
- `BENCH-FUNC-001`: `npm run bench -- --adapter=hajimi-diff --dataset=ai-chat` 执行无报错
- `BENCH-FUNC-002`: 输出 JSON 包含 `compression_ratio`, `speed_mbps`, `correctness: true`
- `BENCH-FUNC-003`: Hajimi-Diff 在 AI 对话场景压缩率 > 80%（对标 zstd）
